...
class OllamaFunctions(ChatOllama):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            try:
                chat_generation_content = chat_generation_content.replace("<|reserved_special_token_5|>system ", "")
                chat_generation_content = chat_generation_content.replace("\n", "")
                chat_generation_content = chat_generation_content.replace("\r", "")
                chat_generation_content = chat_generation_content.replace("\t", "")
                parsed_chat_result = json.loads(chat_generation_content)
            # except json.JSONDecodeError:
            #     parsed_chat_result = { "response": chat_generation_content } # Add content to the response
