...
def describe_updateLmProviderProvider():
    def test_updateLmProviderProvider_OPENAI_API_KEY():
        request = _create_empty_request()
        request.properties = { "OPENAI_API_KEY": "mock_OPENAI_API_KEY" }
        _run_updateLmProviderProvider(request, 'OPENAI_API_KEY', 'mock_OPENAI_API_KEY')

    def test_updateLmProviderProvider_OPENAI_MODELS_DEFAULT_WEIGHT():
        request = _create_empty_request()
        request.weight = 12345
        _run_updateLmProviderProvider(request, 'OPENAI_MODELS_DEFAULT_WEIGHT', '12345')

    def test_updateLmProviderProvider_OPENAI_MODELS_EMBEDDING():
        request = _create_empty_request()
        request.embeddingModels = ["embedding-01", "embedding-02"]
        _run_updateLmProviderProvider(request, 'OPENAI_MODELS_EMBEDDING', 'embedding-01,embedding-02')

    def test_updateLmProviderProvider_OPENAI_MODELS_CONVERSATIONAL():
        request = _create_empty_request()
        request.conversationalModels = ["conversational-01", "conversational-02"]
        _run_updateLmProviderProvider(request, 'OPENAI_MODELS_CONVERSATIONAL', 'conversational-01,conversational-02')

    def test_updateLmProviderProvider_OPENAI_MODELS_VISION():
        request = _create_empty_request()
        request.visionModels = ["vision-01", "vision-02"]
        _run_updateLmProviderProvider(request, 'OPENAI_MODELS_VISION', 'vision-01,vision-02')

    def test_updateLmProviderProvider_OPENAI_MODELS_TOOLS():
        request = _create_empty_request()
        request.toolsModels = ["tools-01", "tools-02"]
        _run_updateLmProviderProvider(request, 'OPENAI_MODELS_TOOLS', 'tools-01,tools-02')
